...
协处理器一词听起来非常高级，~~听起来难度相当于实现0.5个TinyMIPS~~。事实上协处理器是由一系列寄存器和其读写控制逻辑组合而成的一个稍微高级一些的寄存器堆，其中的寄存器反映着处理器运行的状态，也是上层软件对于处理器行为的控制接口。CP0寄存器与通用寄存器，PC寄存器一样，同属于ISA级处理器状态的一部分，程序员可见。
...
![image-20221012201559958](module.assets/image-20221012201559958.png)
